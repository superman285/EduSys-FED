    ...
    <el-card>
      ...
      <el-form label-width="80px">
        ...
        <div v-show="activeStep === 1">
          <el-form-item label="课程封面">
            ...
            <!--
              ...
              -->
            <course-image v-model="course.courseListImg" :limit="4" />
          </el-form-item>
        ...
        <div v-show="activeStep === 4">
          ...
          <el-form-item>
            <el-button type="primary" @click="handleSave($router)"
              >保存
            </el-button>
          </el-form-item>
        </div>
...
<script lang="ts">
import { createApp, defineComponent, reactive, ref, toRefs } from 'vue'
import {
...
const loadCourse = async (courseId: string | number) => {
  ...
  let prop: keyof GetCourseData
  for (prop in data.data) {
    ;(course as any)[prop] = data.data[prop]
  }
}

const handleSave = async (router: Router) => {
  const { data } = await createOrUpdateCourse(course as any)
  if (data.code === '000000') {
